...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="/home/konfi/robocon_yamanashi/Yamanashi_Robocon/nguyen/src/yolo_model/detect_5class_v8n/weights/best.engine", help="YOLOv8モデルパス")
...
def setup_realsense(width: int, height: int, fps: int):
    ...
    print("[INFO] RealSense Camera 初期化完了")
...
def draw_detections(color_image: np.ndarray, results, depth_frame) -> np.ndarray:
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            # ラベル
            label = f"{cls_id}:{cls_name} {conf:.2f} | {depth_str}"

...
for w, h, fps in candidates:
    if any((W == w and H == h and F == fps) for (W, H, F, _) in depth_profiles) and any(
        (W == w and H == h and F == fps) for (W, H, F, _) in color_profiles
    ):
        use = (w, h, fps)
        break
if use is None:
...
try:
    while True:
        ...
        vis = cv2.applyColorMap(
            cv2.convertScaleAbs(dep, alpha=255 / 500), cv2.COLORMAP_JET
        )
